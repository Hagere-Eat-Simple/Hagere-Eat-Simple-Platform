...
if (mysqli_num_rows($result2) > 0) {
  ...
  while($row = mysqli_fetch_assoc($result2)) {
    $count++;
        ...
        $connection;
         $sql= "SELECT * , SUM(amount) tamount FROM orders where resturant= '$value' group by EXTRACT(year FROM date)  order by date asc;";
           ...
           if ($result){
             while($row = $result->fetch(PDO::FETCH_BOTH)){
                       $date = (int)substr($row['date'],0,4);
                       $tamount = intval($row['amount']);
                 echo<<<end_
                         ,['$date',$tamount] 
                         end_;
...
?> -->
    ...
    <?php
        ...
        $connection;
         $sql= "SELECT * , AVG(amount) tamount FROM orders where resturant = '$value' and EXTRACT(month FROM date) = EXTRACT(month FROM CURDATE()) group by EXTRACT(month FROM date),food;";
           $result = $connection->query($sql);
           echo<<< end_
                      <script type="text/javascript">
                      google.charts.load("current", {packages:["corechart"]});
...
<section class="dishes" id="dishes" style="background-color:rgba(105,105,105,0.7);  margin-top:30px;">
   ...
   <?php
      ...
      $connection;
       ...
       // $pass = $_POST["password"];
        $sql = "SELECT * , SUM(amount) tamount FROM orders  INNER JOIN menu ON orders.food = menu.food
        where resturant= '$value' and date = CURDATE() group by price order by amount desc ;  ";
        ...
        if ($result){
          ...
          $add = <<<END_
                    ...
                    <tr>
                    <td>
                    <h3 class="food-title" id="f1" name="$row[food]" >$row[food]</h3><!--class-->
                    </td>
                    <td><div class="shop-item-details">                  
                      <h4><span class="food-price">$row[tamount]</span></h4>     
                    
                     
                      </div> </td><td ><h4>$row[price] birr</h4></td>
                ...
                if(isset($_SESSION['sdate'])){
                  ...
                  $sql1= "SELECT * , SUM(amount) tamount FROM orders INNER JOIN menu ON orders.food = menu.food where resturant= '$value' and  date = '$date'  
                  group by price order by date desc ;  ";
                    $_SESSION['ddate'] =  $_SESSION['sdate'];
                    unset($_SESSION['sdate']);
                }
                else
                   {$sql1= "SELECT * , SUM(amount) tamount FROM orders  
               ...
               $add1 = <<<END_
                          ...
                          <tr>
                          <td>
                         <h3 class="food-title" id="f1" name="$row[food]" >$row[date] </h3><!--class-->
                         </td>
                         <td>                 
                          <h4> <span class="food-title">$row[food] </span> </h4>   
                           </td> <td>
                           <h4> <span class="food-title">$row[tamount]</span></h4> </td>
