...
if (isset($_POST['activate'])){
    ...
    $run = mysqli_query($con,$qrry);
        header("Location:suspended_customers.php");
...
<head>
<!--    style starts-->
    <link rel="stylesheet" href="../resources/css/suspended_cust.css" />
...
<?php
$query1 = "SELECT `username` FROM `customer` where `flag`=2 and `active`=0";
$query1_run = mysqli_query($con, $query1);
$row1 = mysqli_num_rows($query1_run);
echo "<span class='echo'> $row1 </span>";
?>
...
$query = "select username,email,phone,fname,lname from customer where flag=2 and active=0;";
$query_run = mysqli_query($con,$query);
