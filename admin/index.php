...
if (isset($_POST['remove'])){
    $id = filter_input(INPUT_POST,'delete_id',FILTER_SANITIZE_ADD_SLASHES);
...
if (isset($_POST['clist'])){
    ...
    date_default_timezone_set('Africa/Addis_Ababa');
    $curr_date=date('d-m-y');;
    $fileName="Vendor List Table(".$curr_date.").csv";
    header("Content-type: text/x-csv");
     header("Content-Disposition: attachment; filename=".$fileName);
    exit;
    header("Location:index.php");
    ...
    date_default_timezone_set('Africa/Addis_Ababa');
    $curr_date=date('d-m-y');;
    $fileName="Customers List Table(".$curr_date.").csv";
    header("Content-type: text/x-csv");
     header("Content-Disposition: attachment; filename=".$fileName);
    exit;
    header("Location:index.php");
    ...
    date_default_timezone_set('Africa/Addis_Ababa');
    $curr_date=date('d-m-y');;
    $fileName="Orders List Table(".$curr_date.").csv";
    header("Content-type: text/x-csv");
     header("Content-Disposition: attachment; filename=".$fileName);
    exit;
    header("Location:index.php");
...
<div class="navbar">
 ...
 <span class="nav_item"><a href="import.php">Import</a></span>
...
<form method="post" action="" enctype="multipart/form-data">
  ...
  <span class="drop"><button class="report">Export Table</button>
 <div class="dropdown-content">
<form method="post" action="" enctype="multipart/form-data">
  <button type="submit" name="vtable">Vendor Table</button>
  </form>
  <form method="post" action="" enctype="multipart/form-data">
  <button type="submit" name="ctable">Customer Table</button>
  </form>
   <form method="post" action="" enctype="multipart/form-data">
  <button type="submit" name="otable">Order Table </button>
  </form>
  <form method="post" action="" enctype="multipart/form-data">
  <button type="submit" name="dtable">Deposit Table </button>
  </form>
  </div>
</span> 
...
<span class="create_item"><a href="admin_create.php">Create Vendor</a></span>
<span class="create_item"><a href="suspended_vendors.php">Suspended Vendors</a></span>
...
<h1 id="h1">Active Vendors Information</h1>
<?php
$query1 = "SELECT `username` FROM `customer` where `flag`=1 and `active`=1 OR `active`=2";
$query1_run = mysqli_query($con, $query1);
$row1 = mysqli_num_rows($query1_run);
echo "<span class='echo'> $row1 </span>";
?>
<?php
$query = "select customer.username,email,phone,Rname,location,about,logo,active from customer inner join resturant on customer.username=resturant.username and active=1";
$query_run = mysqli_query($con,$query);
...
<table>
    ...
    <tr>
        ...
        <td><?php echo stripslashes($row['Rname']);  ?></td>
        <td><?php echo $row['email'];  ?></td>
        <td><?php echo $row['phone'];  ?></td>
        <td> <a target="_blank" href="<?php echo $row['location'];  ?>">Map</a>
            </td>
        <!-- <td><?php // echo $row['vendor_logo']; ?></td> -->
        <td>
           <form action = "edit.php" method = "post">
               <input type = "hidden" name = "edit_id" value = "<?php echo $row['username'];  ?>">
               <button name = "edit" class="update_btn" type="submit">UPDATE</button>
           </form>
        </td>
        <td>
            <form action = "index.php" method = "post">
                <input type = "hidden" name = "delete_id" value = "<?php echo $row['username'];  ?>">
            <button class="remove_btn" type = "submit" name = "remove" onclick="return confirm('Are you sure you want to deactivate?');">DEACTIVATE</button>
            </form>
        </td>
    </tr>
            <?php
    ...
    function moonIcon() {
     ...
     function alterMenu(){
      ...
      xhttp.onload = function() {
        ...
        var url = "search/search1.php?searchQuery="+document.getElementById("search").value;  
        else
        var url = "search/search1.php";  
